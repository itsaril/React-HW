...
async function getAll(query = '') {
  ...
  // Additional validation: Ensure query is alphanumeric
  const isValidQuery = /^[a-zA-Z0-9\s]+$/.test(query);
  if (!isValidQuery) {
    throw new Error('Query must contain only letters, numbers, or spaces');
  }
  ...
  try {
    ...
    console.error('Error fetching book list:', error, 'Query:', query);
